...
class TransactionPolicy
{
    ...
    /**
     * Retry a single-statement operation on deadlock WITHOUT wrapping in a transaction.
     *
     * Use this for individual INSERTs/UPDATEs that are already atomic by
     * auto-commit but may hit deadlocks from concurrent writers (e.g. chat
     * notification workers inserting email_tracking rows simultaneously).
     *
     * Unlike atomic(), this does NOT wrap in BEGIN/COMMIT — the statement
     * is already atomic on its own. It only adds deadlock detection + retry.
     *
     * If already inside an explicit transaction, retrying is futile (MySQL
     * rolls back the entire transaction on deadlock), so this re-throws
     * immediately.
     *
     * @param callable $operation The single-statement operation
     * @param string $description Description for logging
     * @param int|null $maxRetries Override default max retries
     * @return mixed The operation result
     * @throws Throwable If deadlock persists or non-deadlock error occurs
     */
    public static function retryOnDeadlock(callable $operation, string $description = 'single statement', ?int $maxRetries = null): mixed
    {
        // Inside an explicit transaction, MySQL rolls back the ENTIRE
        // transaction on deadlock — retrying just this statement is futile.
        if (DB::transactionLevel() > 0) {
            return $operation();
        }
        ...
        while ($attempts < $maxRetries) {
            try {
                ...
                if (self::isDeadlock($e)) {
                    ...
                    if ($attempts < $maxRetries) {
                        $delayMs = self::$baseDelayMs * pow(2, $attempts - 1);
                        Log::warning('TransactionPolicy: Deadlock on single statement, retrying', [
                    ...
                    Log::error('TransactionPolicy: Deadlock persisted on single statement after all retries', [
                        'description' => $description,
                        'attempts' => $attempts,
                        'error' => $e->getMessage(),
                    ]);
        ...
        throw $lastException ?? new RuntimeException('retryOnDeadlock operation failed');
    }
