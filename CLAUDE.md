## CRITICAL: File Writing in Docker-Mounted Directories

**NEVER use the Write tool for files inside `iznik-batch/`** - it creates empty files instead of writing content, then changes ownership to root. This has caused repeated data loss.

For files in docker-mounted directories (like `iznik-batch/`):
1. Use `docker exec freegle-batch bash -c 'cat << EOF > /path/to/file ... EOF'`
2. Or use `docker cp` to copy files into the container
3. The Edit tool may work for existing files but verify the result

This is a sandbox/permissions issue with Claude Code's Write tool in this environment.
