...
### 2026-01-22 - Options API to Script Setup Migration
- **Status**: 🔄 In progress - 4 components migrated
- **Branch**: `feature/mcp-log-analysis`
- **Completed**:
  - Set up Vitest unit testing infrastructure with happy-dom environment
  - Fixed nuxt.config.ts process.exit() issue that was killing vitest
  - Migrated 4 components from Options API to script setup with TDD:
    - `DiffPart.vue` - simple diff display component (12 tests)
    - `ModClipboard.vue` - clipboard copy button (8 tests)
    - `ScrollToTop.vue` - scroll-to-top button (11 tests)
    - `ModChangedMapping.vue` - change display component (9 tests)
  - Added CircleCI config for unit tests (freegle/tests@1.1.135)
  - Updated `plans/options-api-migration-design.md` with implementation learnings
- **Test Infrastructure**:
  - `vitest.config.mts` - Simple config with @vitejs/plugin-vue and happy-dom
  - `tests/unit/setup.ts` - Global stubs for Bootstrap and Nuxt components
  - Total: 40 tests passing
- **Key Learnings**:
  - Full Nuxt test environment has compatibility issues with complex nuxt.config.ts
  - Simple @vue/test-utils + happy-dom works well for basic components
  - Need explicit Vue imports (computed, ref, etc.) without Nuxt auto-imports
  - Vue Test Utils wraps multi-root components in a div (use .find() to locate elements)
- **Files Modified**:
  - `iznik-nuxt3/vitest.config.mts` - Test configuration
  - `iznik-nuxt3/tests/unit/setup.ts` - Test setup file
  - `iznik-nuxt3/nuxt.config.ts` - Skip process.exit() in test mode
  - `iznik-nuxt3/modtools/components/DiffPart.vue` - Migrated to script setup
  - `iznik-nuxt3/modtools/components/ModClipboard.vue` - Migrated to script setup
  - `iznik-nuxt3/modtools/components/ScrollToTop.vue` - Migrated to script setup
  - `iznik-nuxt3/modtools/components/ModChangedMapping.vue` - Migrated to script setup
  - `.circleci/orb/freegle-tests.yml` - Added unit test step
  - `plans/options-api-migration-design.md` - Updated with learnings
- **Next**: Continue migrating Tier 1/2 components

### 2026-01-21 - Add Live Database Credential Support
- **Status**: ✅ Infrastructure complete, ready for live testing
- **Branch**: `feature/mcp-log-analysis`
- **Completed**:
  - Added custom database credential support (sqlUrl, dbUser, dbPassword, dbName) to ai-sanitizer
  - Updated mcp-db-server.js to pass credentials via MCP config environment variables
  - Updated server.js and claude-runner.js to flow credentials from frontend to MCP
  - Rebuilt and restarted ai-sanitizer and ai-support-helper containers
  - Set up port-relay.js autostart in ~/.bashrc
  - Copied port-relay.js to permanent location ~/scripts/
  - Verified credential passing mechanism works (tested with local root/iznik credentials)
- **Infrastructure Status**:
  - SSH tunnel working (localhost:1234)
  - Port relay running (0.0.0.0:11234 → localhost:1234 for MySQL)
  - Autostart configured in ~/.bashrc
- **Files Modified**:
  - `support-tools/ai-sanitizer/server.js` - Added sqlUrl, dbUser, dbPassword, dbName parameters
  - `ai-support-helper/mcp-db-server.js` - Added SQL_URL, DB_USER, DB_PASSWORD, DB_NAME env vars
  - `ai-support-helper/server.js` - Extract credentials from request body
  - `ai-support-helper/claude-runner.js` - Inject credentials into MCP config
  - `~/.bashrc` - Added port-relay.js autostart
- **To Test with Live Database**:
  - Configure in ModTools Support settings: SQL URL=`host.docker.internal:11234`, plus live DB credentials
  - Then search for `edward@ehibbert.org.uk`
